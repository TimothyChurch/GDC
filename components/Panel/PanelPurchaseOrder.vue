          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <UFormField label="Tax Rate (%)" name="taxRate">
                <UInput
                  ...
                  @update:model-value="
                    (v: string) =>
                      (localData.taxRate = parseFloat(v) / 100 || 0)
                  "
                  type="number"
                  step="0.01"
                  class="w-full"
            ...
            >
              ...
              <div class="text-xs text-parchment/70">
                <span class="font-semibold text-green-400"
                  >Inventory will be updated automatically</span
                >
                when this order is saved as Delivered. Stock levels for
                {{ localData.items.length }} item{{
                  localData.items.length !== 1 ? "s" : ""
                }}
            ...
            <UFormField label="Items">
              <div class="space-y-2">
                ...
                <div class="space-y-2 border border-white/10 rounded p-2">
                  ...
                  <label
                    class="flex items-center gap-2 text-sm text-parchment/70 cursor-pointer"
                  >
            ...
            <div class="space-y-1 text-sm">
              ...
              >
                <span
                  >Tax ({{
                    ((localData.taxRate || 0) * 100).toFixed(2)
                  }}%)</span
                >
                <span>{{ Dollar.format(totalTax) }}</span>
              </div>
              <div
                v-if="(localData.shipping || 0) > 0"
                class="flex justify-between text-parchment/60"
              >
                <span>Shipping</span>
              ...
              <div
                class="flex justify-between text-lg font-bold text-parchment pt-1 border-t border-white/10"
              >
                <span>Total</span>
