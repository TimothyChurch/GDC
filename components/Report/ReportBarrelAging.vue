...
const barrelData = computed(() => {
  return vesselStore.barrels.map(barrel => {
    ...
    const entry = getStage(batch, 'barrelAging')?.entry
    const fillDate = entry?.date ? new Date(entry.date) : null
    const ageDays = fillDate ? differenceInDays(new Date(), fillDate) : 0
    const entryVolume = entry?.volume || barrel.current?.volume || 0
