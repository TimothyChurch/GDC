...
import { PIPELINE_TEMPLATES, STAGE_DISPLAY, getAvailableStages, stageTextColor, stageBgColor } from '~/composables/batchPipeline';
...
const display = (name: string) => STAGE_DISPLAY[name] || { icon: 'i-lucide-circle', color: 'neutral' };
</script>
...
<template>
  <div class="space-y-3">
    ...
    <div v-if="pipeline.length > 0" class="space-y-1.5">
      <div class="text-xs text-parchment/60 uppercase tracking-wider">Pipeline Stages</div>
      <div class="space-y-1">
        <div
        ...
        >
          ...
          <span class="text-[10px] text-parchment/40 tabular-nums">{{ index + 1 }}</span>
          <div class="flex items-center gap-0.5">
            ...
            <UButton
              ...
              variant="ghost"
              size="2xs"
              :disabled="index >= pipeline.length - 2 || pipeline[index + 1] === 'Bottled'"
              @click="moveStage(index, 1)"
            />
            <UButton
              v-if="stage !== 'Bottled'"
    ...
    >
      <UIcon name="i-lucide-route" class="text-2xl text-parchment/20 mx-auto mb-2" />
      <p class="text-sm text-parchment/50">Select a template or add stages below</p>
    </div>

    <!-- Add stage -->
    <div v-if="availableStages.length > 0" class="space-y-1.5">
      <div class="text-xs text-parchment/60 uppercase tracking-wider">Add Stage</div>
      <div class="flex flex-wrap gap-1.5">
        ...
        >
          <UIcon :name="display(stage).icon" :class="stageTextColor(display(stage).color)" />
          {{ stage }}
        </UButton>
      </div>
    </div>
