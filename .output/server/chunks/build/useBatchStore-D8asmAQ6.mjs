import { h as hasStageVolumes, i as isStageActive, g as getActiveStages, a as getPreviousStage, b as getStageIndex, S as STAGE_KEY_MAP } from './batchPipeline-Dr1IalWc.mjs';
...
const useBatchStore = defineStore("batches", () => {
  ...
  const addStrippingRun = async (batchId, run) => {
    const target = crud.items.value.find((b) => b._id === batchId);
    if (!target) return;
  ...
  const updateStrippingRun = async (batchId, runIndex, data) => {
    const target = crud.items.value.find((b) => b._id === batchId);
  ...
  const addSpiritRun = async (batchId, run) => {
    const target = crud.items.value.find((b) => b._id === batchId);
    if (!target) return;
  ...
  const updateSpiritRun = async (batchId, runIndex, data) => {
    const target = crud.items.value.find((b) => b._id === batchId);
  ...
  const updateRunByStageKey = async (stageKey, batchId, runIndex, data) => {
...
//# sourceMappingURL=useBatchStore-D8asmAQ6.mjs.map
