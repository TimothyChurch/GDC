...
<objective>
Optimization auditor for the GDC project's Claude Code configuration. Audits the current state and applies fixes to minimize token waste while maintaining performance.
</objective>

<quick_start>
- `/optimize` — Full audit (all sections)
- `/optimize memory` — Memory files only (Sections 1-2)
- `/optimize config` — CLAUDE.md, rules, .claudeignore (Sections 3-5)
- `/optimize agents` — Agent configs and agent memory (Sections 6-7)
- `/optimize report` — Generate report only, no changes
</quick_start>
...
Read all files in the memory directory and check each file for staleness. Remove files that are no longer relevant.
...
<success_criteria>
- All targeted sections audited thoroughly
- Stale/redundant content identified and fixed (or flagged for report-only mode)
- Summary report generated with token impact estimates
- No content removed that is still accurate and useful
- Files remain well-structured after edits
</success_criteria>
